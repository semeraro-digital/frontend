...
const loadCorse = async () => {
  try {
    ...
    const response = await axios.post(`${API_BASE_URL}/assegnazioni/assegnazionirange`, payload);
...
const aggiungiCorsa = async () => {
  try {
    ...
    request.datapartenza = moment(request.datapartenza)
      .hours(request.orapartenza.hours)
      .minutes(request.orapartenza.minutes)
      .format();
